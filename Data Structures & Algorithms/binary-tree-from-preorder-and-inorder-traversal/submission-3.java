...
class Solution {
    ...
    public TreeNode build(int preStart , int preEnd, int[] preorder , int inStart, int inEnd, int[] inorder){
        ...
        root.left = build(preStart + 1 , preStart + countLeft , preorder , inStart , rootIndx-1 , inorder);
        root.right = build(preStart +countLeft +1,preEnd , preorder, rootIndx+1,  inEnd , inorder);
